...
class DocumentParser:
    ...
    def _init_docling(self) -> None:
        ...
        try:
            ...
            logger.warning("docling_not_available", message="Docling is not installed. Use: pip install docling")
            self.docling_available = False
    ...
    def _parse_simple(self, content: str, filename: str, content_type: str) -> str:
        ...
        if isinstance(content, str):
            ...
            def looks_like_base64(s: str) -> bool:
                """Check if string looks like base64 encoded data."""
                if len(s) < 100:  # Base64 encoded data is usually longer
                    return False
                # Base64 uses A-Z, a-z, 0-9, +, /, = (with padding)
                base64_chars = set('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=')
                # Check if most characters are base64 (allow some whitespace)
                non_base64 = sum(1 for c in s if c not in base64_chars and not c.isspace())
                return non_base64 < len(s) * 0.1  # Less than 10% non-base64 chars
            ...
            if looks_like_base64(content):
                try:
                    ...
                    if decoded.startswith(b'%PDF'):
                        ...
                        logger.warning("pdf_binary_detected", message="Binary PDF detected. Install Docling to extract text.")
                        return ""
                    else:
                    ...
                    logger.warning("base64_decode_failed", message="Failed to decode base64, treating as plain text")
                    return content
            else:
